...
namespace Santa.Api.Controllers
{
    ...
    public class ProfileController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<MessageHistory>>> GetUnloadedHistories(Guid clientID)
        {
            ...
            if (logicStaticClient.clientID == checkerClient.clientID)
            {
                return Ok((await repository.GetUnloadedProfileChatHistoriesAsync(checkerClient.clientID)).OrderBy(h => h.eventType.eventDescription).ThenBy(h => h.conversationClient.clientNickname));
            }
            else
            {
                return StatusCode(StatusCodes.Status401Unauthorized);
