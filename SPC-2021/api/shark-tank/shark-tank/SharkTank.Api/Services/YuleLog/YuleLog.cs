...
namespace SharkTank.Api.Services.YuleLog
{
    public class YuleLog : IYuleLog
    {
        ...
        public async Task logCreatedNewAssignments(BaseClient requestingClient, Client sendingClient, List<string> listNewAssignmentNicknames)
        {
            ...
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_ASSIGNMENT_CATEGORY), logMessage));
            await saveLogs();
        }
        public async Task logCreatedNewMessage(BaseClient requestingClient, ClientChatMeta sender, ClientChatMeta receiver)
        {
            ...
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_NEW_MESSAGE_CATEGORY), logMessage));
            await saveLogs();
        }
        public async Task logCreatedNewClient(BaseClient requestingClient, Client newClient)
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_NEW_CLIENT_CATEGORY), $"{requestingClient.nickname} requested to make a new client object with email: {newClient.email}"));
            await saveLogs();
        }

        public async Task logCreatedNewAuth0Client(BaseClient requestingClient, string createdAuth0AccountEmail)
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_NEW_AUTH0_CLIENT_CATEGORY), $"{requestingClient.nickname} requested to make a client an Auth0 account for the email: {createdAuth0AccountEmail}"));
            await saveLogs();
        }

        public async Task logCreatedNewTag(BaseClient requestingClient, Logic.Objects.Tag newTag)
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_NEW_TAG_CATEGORY), $"{requestingClient.nickname} requested to make a new tag: {newTag.tagName}"));
            await saveLogs();
        }

        public async Task logCreatedNewClientTagRelationships(BaseClient requestingClient, BaseClient targetClient)
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.CREATED_NEW_CLIENT_TAG_RELATIONSHIPS_CATEGORY), $"{requestingClient.nickname} requested to add tags to {targetClient.nickname}"));
            await saveLogs();
        }
        #endregion
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.GET_ALL_CLIENT_CATEGORY), requestingClient.nickname + " made a request to retrieve a list of all clients"));
            await saveLogs();
        }
        ...
        public async Task logModifiedAnswer(BaseClient requestingClient, Question questionBeingAnsweredFor, string oldAnswer, string newAnswer)
        {
            ...
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.MODIFIED_ANSWER_CATEGORY), logMessage));
            await saveLogs();
        }
        public async Task logModifiedAssignmentStatus(BaseClient requestingClient, string assignmentNickname, AssignmentStatus oldStatus, AssignmentStatus newStatus)
        {
            ...
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.MODIFIED_ASSIGNMENT_STATUS_CATEGORY), logMessage));
            await saveLogs();
        }
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.GET_ALL_HISTORY_CATEGORY), $"{requestingClient.nickname} modified data on a profile for {modifiedProfile.nickname}"));
            await saveLogs();
        }
        public async Task logModifiedClient(BaseClient requestingClient, Client modifiedClient)
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.GET_ALL_HISTORY_CATEGORY), $"{requestingClient.nickname} modified a client's data for {modifiedClient.nickname}"));
            await saveLogs();
        }
        public async Task logModifiedMessageReadStatus(BaseClient requestingClient, Message markedMessage)
        {
            ...
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.MODIFIED_MESSAGE_READ_STATUS_CATEGORY), $"{requestingClient.nickname} requested to mark a past message as read where the sender Client was {markedMessage.senderClient.clientNickname} and reciever was {receiverNickname}"));
            await saveLogs();
        }
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.MODIFIED_CLIENT_STATUS_CATEGORY), $"{requestingClient.nickname} requested to modify {affectedClientWithNewStatus.nickname}'s status from '{oldStatus.statusDescription}' to '{affectedClientWithNewStatus.clientStatus.statusDescription}'"));
            await saveLogs();
        }
        #endregion
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.DELETED_CLIENT_CATEGORY), $"{requestingClient.nickname} requested to delete {deletedClient.nickname} and all related entities"));
            await saveLogs();
        }
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.DELETED_ASSIGNMENT_CATEGORY), $"{requestingClient.nickname} requested to remove {deletedAssignment.relationshipClient.clientNickname} from {affectedClient.nickname}'s assignment list"));
            await saveLogs();
        ...
        {
            await repository.CreateNewLogEntry(makeLogTemplateObject(await getCategoryByName(SharkTankConstants.DELETED_TAG_CATEGORY), $"{requestingClient.nickname} requested to delete the {deletedTag.tagName} tag"));
            await saveLogs();
        }
        #endregion
        ...
        {
            Category errorCategory = await getCategoryByName(SharkTankConstants.MODIFIED_ANSWER_CATEGORY);
            string logMessage = $"{requestingClient.nickname}'s request failed. ";

            #region post
            if (errorCategory.categoryName == SharkTankConstants.CREATED_ASSIGNMENT_CATEGORY)
            {
                logMessage += "There was an error posting new assignments";
            }
            if (errorCategory.categoryName == SharkTankConstants.CREATED_NEW_MESSAGE_CATEGORY)
            {
                logMessage += "There was an error creating a new message";
            }

            if (errorCategory.categoryName == SharkTankConstants.CREATED_NEW_CLIENT_CATEGORY)
            {
                logMessage += "There was an error creating a new client object";
            }
            if (errorCategory.categoryName == SharkTankConstants.CREATED_NEW_AUTH0_CLIENT_CATEGORY)
            {
                logMessage += "There was an error creating a new Auth0 account for a client";
            }
            if (errorCategory.categoryName == SharkTankConstants.CREATED_NEW_TAG_CATEGORY)
            {
                logMessage += "There was an error creating a new tag";
            }
            if (errorCategory.categoryName == SharkTankConstants.CREATED_NEW_CLIENT_TAG_RELATIONSHIPS_CATEGORY)
            {
            ...
            #region get
            if (errorCategory.categoryName == SharkTankConstants.GET_ALL_CLIENT_CATEGORY)
            {
                logMessage += "There was an error getting all clients";
            }
            if (errorCategory.categoryName == SharkTankConstants.GET_PROFILE_CATEGORY)
            {
                logMessage += "There was an error retrieving a profile";
            }
            if (errorCategory.categoryName == SharkTankConstants.GET_SPECIFIC_CLIENT_CATEGORY)
            {
                logMessage += "There was an error getting the client data";
            }
            if (errorCategory.categoryName == SharkTankConstants.GET_SPECIFIC_HISTORY_CATEGORY)
            {
                logMessage += "There was an error getting the history object";
            }
            if (errorCategory.categoryName == SharkTankConstants.GET_ALL_HISTORY_CATEGORY)
            {
            ...
            #region put
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_ANSWER_CATEGORY)
            {
                logMessage += "There was an error changing the answer";
            }
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_ASSIGNMENT_STATUS_CATEGORY)
            {
                logMessage += "There was an error modifying the assignment's status";
            }
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_CLIENT_CATEGORY)
            {
                logMessage += "There was an error modifying a client";
            }
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_PROFILE_CATEGORY)
            {
                logMessage += "There was an error modifying the profile";
            }
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_MESSAGE_READ_STATUS_CATEGORY)
            {
                logMessage += "There was an error modifying the read status of a message";
            }
            if (errorCategory.categoryName == SharkTankConstants.MODIFIED_CLIENT_STATUS_CATEGORY)
            {
            ...
            #region delete
            if (errorCategory.categoryName == SharkTankConstants.DELETED_CLIENT_CATEGORY)
            {
                logMessage += "There was an error deleting a client and all the related entities";
            }
            if (errorCategory.categoryName == SharkTankConstants.DELETED_ASSIGNMENT_CATEGORY)
            {
                logMessage += "There was an error deleting an assignment from a client's list";
            }
            if (errorCategory.categoryName == SharkTankConstants.DELETED_TAG_CATEGORY)
            {
